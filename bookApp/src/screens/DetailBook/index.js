...
export default function DetailBook({navigation}) {
  ...
  const loveFeatured = () => {
    ...
    notifikasi.kirimNotif("1", "Lope Lope", "Makasih sudah memberi hati kepadaku <3");
  }
    ...
    style={{backgroundColor: tema === 'light' ? '#a7cbad' : 'black'}}>
      ...
      <View style={styles.cardTop}>
        ...
        <View style={styles.textBook}>
          <Text style={{fontSize: ms(15),marginBottom: ms(10), fontWeight:'bold', color: 'black'}}>{detailMovie.title}</Text>
      ...
      <View style={styles.cardMid}>
        <View style={{alignItems:'center'}}>
          ...
          <TouchableOpacity style={styles.buttonBuy}>
            ...
            prefix="Buy Rp "
            renderText={formattedValue => <Text style={{color:'#fff'}}>{formattedValue}</Text>} />
          </TouchableOpacity>
      </View>
      <View style={{ marginHorizontal: ms(10), marginTop: ms(20), marginBottom: ms(10) }}>
        <Text style={{fontSize: ms(14), fontWeight:'bold'}}>Overview</Text>
      </View>
      <View style={styles.synopsis}> 
        <Text style={{textAlign: 'justify',lineHeight: 20,}}>{detailMovie.synopsis}</Text>
      </View>
    </ScrollView>
  )
}
